...
class Swarm {
 ...
 public:
  ...
  Point separation(std::vector<Boid>::iterator iti) {
    Point added_velocity{0, 0};
    for (auto itj = m_boids.begin(); itj < m_boids.end(); ++itj) {
      if (std::distance(itj, iti) != 0 &&
          (iti->r - itj->r).distance() < constants::separation_distance) {
  ...
  Point cohesion(std::vector<Boid>::iterator iti) {
    Point added_velocity{0, 0};
    int in_range{0};
    for (auto itj = m_boids.begin(); itj < m_boids.end(); ++itj) {
      if (std::distance(itj, iti) != 0 &&
          (iti->r - itj->r).distance() < constants::distance_coefficent) {
    ...
    if (in_range) {
      added_velocity = constants::cohesion_coefficent *
                       ((1. / (in_range)) * added_velocity - (iti->r));
    }
    return added_velocity;
  }

  Point alignment(std::vector<Boid>::iterator iti) {
    Point added_velocity{0, 0};
    int in_range{0};
    for (auto itj = m_boids.begin(); itj < m_boids.end(); ++itj) {
      if (std::distance(itj, iti) != 0 &&
    ...
    if (in_range) {
      added_velocity = constants::alignment_coefficent *
                       ((1. / (in_range)) * added_velocity - (iti->v));
    }
    return added_velocity;
  }
  ...
  Point turn_around(std::vector<Boid>::iterator iti) {
    if (iti->r.x() > constants::window_width - constants::margin_width)
      return {-constants::turn_coefficent, 0.};
    if (iti->r.x() < constants::margin_width)
      return {constants::turn_coefficent, 0.};
    if (iti->r.y() > constants::window_height - constants::margin_width)
      return {0., -constants::turn_coefficent};
    if (iti->r.y() < constants::margin_width)
      return {0., constants::turn_coefficent};
    return {0., 0.};
  }
  ...
  void vertex_update(std::vector<Boid>::iterator it) {
    ...
    m_vertices[3 * (it - m_boids.begin())].position =
        ...
        sf::Vector2f((it->r + 2 * forward_vertex).x(),
                     (it->r + 2 * forward_vertex).y());

    // rotate by 120
    forward_vertex.rotate(2. / 3 * constants::pi);

    m_vertices[(3 * (it - m_boids.begin())) + 1].position = sf::Vector2f(
        (it->r + forward_vertex).x(), (it->r + forward_vertex).y());

    forward_vertex.rotate(2. / 3 * constants::pi);

    m_vertices[(3 * (it - m_boids.begin())) + 2].position = sf::Vector2f(
        (it->r + forward_vertex).x(), (it->r + forward_vertex).y());
  }
  ...
  void update(double delta_t) {
    for (auto it = m_boids.begin(); it < m_boids.end(); it++) {
      ...
      if (it->v.distance() < constants::max_velocity) {
        it->v = it->v + separation(it) + cohesion(it) + alignment(it) +
                turn_around(it);
      } else {
        // is using an unidentified object point bad practice?
        it->v = Point{constants::velocity_reduction_coefficent * (it->v.x()),
                      constants::velocity_reduction_coefficent * (it->v.y())};
      }
