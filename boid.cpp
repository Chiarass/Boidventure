...
Point Boid::cohesion(const std::vector<Boid*>& in_range, double cohesion_coeff) {
  Point added_velocity{0., 0.};
  for (auto other_boid : in_range) {
    added_velocity = added_velocity + (other_boid->pos());
  }
  if (!in_range.empty()) {
    added_velocity = cohesion_coeff *
                     ((1. / in_range.size()) * added_velocity - m_pos);
  }
  return added_velocity;
}

Point Boid::alignment(const std::vector<Boid*>& in_range, double alignment_coeff) {
  Point added_velocity{0., 0.};
  for (auto other_boid : in_range) {
    added_velocity = added_velocity + (other_boid->vel());
  }
  if (!in_range.empty()) {
    added_velocity = alignment_coeff *
